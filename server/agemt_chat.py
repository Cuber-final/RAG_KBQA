...
# from configs.model_config import LLM_MODELS, TEMPERATURE, HISTORY_LEN, Agent_MODEL

from langchain.chains import LLMChain
from langchain.memory import ConversationBufferWindowMemory
from langchain.agents import LLMSingleActionAgent, AgentExecutor
from typing import AsyncIterable, Optional, List
from agent.callbacks import CustomAsyncIteratorCallbackHandler
from server.utils import get_prompt_template
from agent.main_agents.glm_agent import initialize_glm3_agent
from agent.tools_select import tools, tool_names
from server.utils import History
...
logger.add("./run_log_agent", format="{time} {level} {message}", level="DEBUG")
...
async def agent_chat(
    ...
    async def agent_chat_iterator(
        ...
        model_container.DATABASE = {
            name: details["kb_info"] for name, details in kb_list.items()
        }
        ...
        if (
            "chatglm3" in model_container.MODEL.model_name
            or "zhipu-api" in model_container.MODEL.model_name
        ):
        ...
        if stream:
            async for chunk in callback.aiter():
                ...
                if data["status"] == Status.start or data["status"] == Status.complete:
                    ...
                    yield json.dumps(
                        {"final_answer": data["final_answer"]}, ensure_ascii=False
                    )
                else:
            ...
            yield json.dumps(
                {"answer": answer, "final_answer": final_answer}, ensure_ascii=False
            )
        await task
