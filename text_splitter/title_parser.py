...
def extrac_title(page_content: str) -> list[str]:
    # 在文本分块前提取标题，重构内容排版
    import re

    reg = r"^(?:[一二三四五六七八九十]|\d){1}\s*[、.]\s*.+"
    pattern = re.compile(reg, re.MULTILINE)
    # content = "一、langchain的开发配置\n2.langchainset\n测试三个无语\n21条命\n2 测试三"
    # 识别出所有标题后遍历原文，逐个提取标题，整理当前标题的下文，组合新内容
    ...
    for title in titles:
        ...
        if title.isnumeric():
            continue
        if len(title.strip().replace(" ", "")) > 30:
            continue
        # 若以标点符号结尾，说明不是标题
        ENDS_IN_PUNCT_PATTERN = r"[^\w\s]\Z"
        ENDS_IN_PUNCT_RE = re.compile(ENDS_IN_PUNCT_PATTERN)
        if ENDS_IN_PUNCT_RE.search(title) is not None:
            continue
